...
module GameUtil {
    export class TipsPanel extends egret.DisplayObjectContainer {
        ...
        public init(): void {
            ...
            if (!this.bDisappear) {
                var coverbg: egret.Shape = GameUtil.createRect(0, 0, GameConfig.getSW(), GameConfig.getSH(), 0.4);
                this.addChild(coverbg);
                this.touchEnabled = true;
            }

            if (this.tipImg == null) {
                var tipbgcover: egret.Shape = GameUtil.createRect(GameConfig.getSW() / 2, GameConfig.getSH() / 2, this.text.width * 2, this.text.height * 1.5, 1, 0x8c8b88);
